...
export class CommonService {
  ...
  /**
   * 根据状态获取安全的html代码，用于状态管道.
   * @param value 状态值
   * @param status 所有状态
   */
  getSafeHtml<T>(value: T, status: { [index: string]: StatusEnum<T> }): SafeHtml {
    let result: SafeHtml = null;
    let description = '';
    for (const key in status) {
      if (status[key]) {
        const v = status[key] as StatusEnum<T>;
        if (v.value === value) {
          description = v.description;
          result = this.domSanitizer.bypassSecurityTrustHtml(`<span class='badge badge-pill badge-${v.clazz}'>${description}</span>`);
        }
      }
    }
    return result;
